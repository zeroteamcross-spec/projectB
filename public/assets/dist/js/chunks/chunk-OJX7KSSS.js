...
</svg>`}function Ln({daftar:a,demo:e}){let t=A.hero;return`
...
</section>`}function $n(){let a=A.masalah,e=a.slides.map(t=>t.map(Bn).join("")).join("|");return`
...
</article>`}function Mn(){let a=A.fitur,e=a.slides.map(t=>t.map(Tn).join("")).join("|");return`
<section class="ck-fitur" id="fitur">
  <div class="ck-lajur">
    <h2 data-reveal>${da(a.judul)}</h2>
    ${Da({nama:"fitur",slides:e,satuLajur:!0})}
  </div>
</section>`}function Tn(a){return`
...
</article>`}function zn(){let a=A.kenapa;return`
...
</section>`}function Rn(){let a=A.testimoni,e=a.slides.map(t=>t.map(Pn).join("")).join("|");return`
<section class="ck-testimoni">
  ${Un()}
  <div class="ck-lajur">
    <h2 data-reveal>${f(a.judul)}</h2>
    <div class="ck-testimoni__bintang" data-reveal aria-label="${a.bintang} dari 5">
      ${Array.from({length:a.bintang},Hn).join("")}
    </div>
    ${Da({nama:"testimoni",slides:e,berpanah:!0})}
  </div>
</section>`}function Pn(a){return`
...
</figure>`}function In(){let a=A.partner;return`
...
</section>`}function Dn({daftar:a,konsultasi:e}){let t=A.penutup;return`
...
</section>`}function On(a){let e=A.footer;return`
...
</button>`}function Hn(){return`
<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
  <path d="M12 2.2l2.9 6.2 6.8.8-5 4.6 1.3 6.7L12 17.2 6 20.5l1.3-6.7-5-4.6 6.8-.8z"/>
</svg>`}function Un(){let a="M12 2.2l2.9 6.2 6.8.8-5 4.6 1.3 6.7L12 17.2 6 20.5l1.3-6.7-5-4.6 6.8-.8z";return`
...
`;function Ze(){if(!document.getElementById(Ve)){let a=document.createElement("style");a.id=Ve,a.textContent=Qn,document.head.append(a)}if(document.getElementById(Xe)||Zn.forEach((a,e)=>{let t=document.createElement("link");e===0&&(t.id=Xe),t.rel="preconnect",t.href=a,t.crossOrigin="anonymous",document.head.append(t)}),!document.getElementById(Ye)){let a=document.createElement("link");a.id=Ye,a.rel="stylesheet",a.href=Xn,document.head.append(a)}document.documentElement.classList.add(K),document.body.classList.add(K)}function Qe(){document.documentElement.classList.remove(K),document.body.classList.remove(K)}function at(){let a=null,e=null,t=null,n=null;return R({mount(){return a=document.createElement("div"),a.id="carlynk_landing_root",a.className="relative w-full",a.innerHTML=Ge({rute:Ue(er(),ar())}),Ze(),r(),a},bindEvents(){!a||e||(e=We(a))},unmount(){e?.(),e=null},dispose(){e?.(),e=null,Qe(),o(),a=null}});function r(){t=document.querySelector("#app header"),t&&(n=t.style.display,t.style.display="none")}function o(){t&&(t.style.display=n??"",t=null,n=null)}}function ar(){let a=Ga("seller");return a&&a!==qa()?`${window.location.protocol}//${a}/login/seller`:""}function er(){let a=String(O.contact?.whatsapp??"").replace(/\D/g,"").replace(/^0/,"62");return a?`https://wa.me/${a}`:""}var I=Object.freeze({daftar:"/daftar-showroom",masuk:"/login/seller"});function tt({namaMerek:a,tagline:e,tautanWhatsapp:t,alamatEtalase:n,logoUrl:r=""}){let o=G(a),i=et(r,o,24,18),s=et(r,o,22,17);return`
...
`}function et(a,e,t,n){let r=String(a||"").trim();return r!==""?`<img data-logo="" src="${G(r)}" alt="${e}" style="height:${t+8}px;width:auto;max-width:190px;object-fit:contain;display:block;transition:transform .5s cubic-bezier(.22,.9,.28,1)">`:`<div data-logo="" style="width:${t}px;height:${t}px;border-radius:7px;background:linear-gradient(135deg,#1e81b0,#eab676);box-shadow:0 0 20px rgba(30,129,176,.5);transition:transform .5s cubic-bezier(.22,.9,.28,1)"></div><span style="font-family:Sora,sans-serif;font-weight:700;font-size:${n}px;letter-spacing:-.03em">${e}</span>`}function G(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var tr=Object.freeze(["halaman","listing","marketing"]);function nt(a){let e=[],t=r=>Array.from(a.querySelectorAll(r)),n=r=>a.querySelector(r);return e.push(nr(t)),e.push(rr(t)),e.push(or(t)),e.push(sr(t)),e.push(dr(n,t)),e.push(lr(t)),e.push(pr(a)),()=>e.splice(0).forEach(r=>r())}function nr(a){let e=[];return a("[data-word]").forEach((t,n)=>{t.style.display="inline-block",t.style.opacity="0",t.style.filter="blur(10px)",t.style.transform="translateY(28px) rotateX(-40deg)",t.style.transition="opacity .8s cubic-bezier(.22,.9,.28,1), transform .9s cubic-bezier(.22,.9,.28,1), filter .8s ease",t.style.transitionDelay=`${200+n*75}ms`,e.push(setTimeout(()=>{t.style.opacity="1",t.style.filter="none",t.style.transform="none"},60))}),()=>e.forEach(t=>clearTimeout(t))}function rr(a){let e=a("[data-reveal]"),t=[];if(!e.length)return()=>{};e.forEach((r,o)=>{r.style.opacity="0",r.style.transform="translateY(26px)",r.style.transition="opacity .9s cubic-bezier(.22,.9,.28,1), transform .9s cubic-bezier(.22,.9,.28,1)",r.style.transitionDelay=`${Math.min(o,4)*80}ms`});let n=new IntersectionObserver(r=>{r.forEach(o=>{if(!o.isIntersecting)return;let i=o.target;i.style.opacity="1",i.style.transform="none",i.querySelectorAll("[data-bar]").forEach((c,d)=>{c.style.animation=`barGrow .85s cubic-bezier(.22,.9,.28,1) ${d*110}ms both`}),i.querySelectorAll("[data-lead]").forEach((c,d)=>{t.push(setTimeout(()=>{c.style.opacity="1",c.style.transform="none"},300+d*420))}),i.querySelectorAll("[data-pop]").forEach((c,d)=>{c.style.opacity="0",c.style.transform="translateY(14px) scale(.96)",t.push(setTimeout(()=>{c.style.opacity="1",c.style.transform="none"},260+d*130))});let s=i.querySelector("[data-type]");s&&ir(s,t);let l=i.querySelector("[data-status]");l&&t.push(setTimeout(()=>{l.textContent="Tayang",l.style.color="#15803d",l.style.background="rgba(21,128,61,.12)"},1600)),i.querySelectorAll("[data-row]").forEach((c,d)=>{c.style.opacity="0",c.style.transform="translateX(-14px)",c.style.transition=`opacity .5s ease ${d*90}ms, transform .5s cubic-bezier(.22,.9,.28,1) ${d*90}ms, background .25s`,requestAnimationFrame(()=>{c.style.opacity="1",c.style.transform="none"})}),n.unobserve(i)})},{threshold:.14,rootMargin:"0px 0px -6% 0px"});return e.forEach(r=>n.observe(r)),()=>{n.disconnect(),t.forEach(r=>clearTimeout(r))}}function ir(a,e){let t=a.getAttribute("data-type")||"",n=0,r=()=>{a.textContent=t.slice(0,n++),n<=t.length&&e.push(setTimeout(r,42))};r()}function or(a){let e=[];return a("[data-tilt]").forEach(t=>{let n=o=>{let i=t.getBoundingClientRect(),s=(o.clientX-i.left)/i.width-.5,l=(o.clientY-i.top)/i.height-.5;t.style.transform=`perspective(1000px) rotateY(${(s*6).toFixed(2)}deg) rotateX(${(-l*6).toFixed(2)}deg) translateY(-5px)`,t.style.transitionDuration=".12s"},r=()=>{t.style.transitionDuration=".5s",t.style.transform="none"};t.addEventListener("pointermove",n),t.addEventListener("pointerleave",r),e.push(()=>{t.removeEventListener("pointermove",n),t.removeEventListener("pointerleave",r)})}),()=>e.forEach(t=>t())}function sr(a){let e=[];return a("[data-magnet]").forEach(t=>{let n=o=>{let i=t.getBoundingClientRect(),s=(o.clientX-(i.left+i.width/2))*.22,l=(o.clientY-(i.top+i.height/2))*.3;t.style.transform=`translate(${s.toFixed(1)}px,${l.toFixed(1)}px)`},r=()=>{t.style.transform="none"};t.addEventListener("pointermove",n),t.addEventListener("pointerleave",r),e.push(()=>{t.removeEventListener("pointermove",n),t.removeEventListener("pointerleave",r)})}),()=>e.forEach(t=>t())}function lr(a){let e=[];return a("[style-hover]").forEach(t=>{let n=cr(t.getAttribute("style-hover"));if(!n.length)return;let r=null,o=()=>{r=n.map(([s])=>[s,t.style.getPropertyValue(s)]),n.forEach(([s,l])=>t.style.setProperty(s,l))},i=()=>{(r||[]).forEach(([s,l])=>{l?t.style.setProperty(s,l):t.style.removeProperty(s)}),r=null};t.addEventListener("pointerenter",o),t.addEventListener("pointerleave",i),e.push(()=>{t.removeEventListener("pointerenter",o),t.removeEventListener("pointerleave",i)})}),()=>e.forEach(t=>t())}function cr(a){return String(a||"").split(";").map(e=>e.trim()).filter(Boolean).map(e=>{let t=e.indexOf(":");return t===-1?null:[e.slice(0,t).trim(),e.slice(t+1).trim()]}).filter(Boolean)}function dr(a,e){let t=a("[data-progress]"),n=a("[data-scrollhint]"),r=e("[data-navlink]"),o=!1,i=()=>{o||(o=!0,requestAnimationFrame(()=>{o=!1;let s=window.scrollY,l=Math.max(document.body.scrollHeight-window.innerHeight,1),c=Math.min(s/l,1);t&&(t.style.width=`${(c*100).toFixed(2)}%`),n&&(n.style.opacity=s>120?"0":"1");let d=-1;tr.forEach((g,p)=>{let m=document.getElementById(`saas_landing_${g}`);m&&m.getBoundingClientRect().top<window.innerHeight*.45&&(d=p)}),r.forEach((g,p)=>{g.style.color=p===d?"#17698F":"rgba(28,25,23,.62)"})}))};return window.addEventListener("scroll",i,{passive:!0}),i(),()=>window.removeEventListener("scroll",i)}function pr(a){let e=t=>{let n=t.target.closest("[data-scroll]");!n||!a.contains(n)||(t.preventDefault(),ur(n.getAttribute("data-scroll")))};return a.addEventListener("click",e),()=>a.removeEventListener("click",e)}function ur(a){let e=document.getElementById(`saas_landing_${a}`);e&&e.scrollIntoView({behavior:document.hidden?"instant":"smooth",block:"start"})}var q="saas-landing-active",rt="saas_landing_style",it="saas_landing_font",ot="saas_landing_font_preconnect",mr="https://fonts.googleapis.com/css2?family=Sora:wght@600;700;800&family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600;9..40,700&family=JetBrains+Mono:wght@400;500&display=swap",gr=["https://fonts.googleapis.com","https://fonts.gstatic.com"],fr=`
...
`;function st(){if(!document.getElementById(rt)){let a=document.createElement("style");a.id=rt,a.textContent=fr,document.head.append(a)}if(document.getElementById(ot)||gr.forEach((a,e)=>{let t=document.createElement("link");e===0&&(t.id=ot),t.rel="preconnect",t.href=a,t.crossOrigin="anonymous",document.head.append(t)}),!document.getElementById(it)){let a=document.createElement("link");a.id=it,a.rel="stylesheet",a.href=mr,document.head.append(a)}document.documentElement.classList.add(q),document.body.classList.add(q)}function lt(){document.documentElement.classList.remove(q),document.body.classList.remove(q)}function ct(){let a=null,e=null,t=null,n=null;return R({mount(){return a=document.createElement("div"),a.id="saas_landing_root",a.className="relative w-full",a.style.background="#FDFDFC",a.innerHTML=tt({namaMerek:O.appName,tagline:O.appTagline,tautanWhatsapp:br(),alamatEtalase:kr(),logoUrl:oe(O.uploadedLogoUrl)}),st(),r(),a},bindEvents(){!a||e||(e=nt(a))},unmount(){e?.(),e=null},dispose(){e?.(),e=null,lt(),o(),a=null}});function r(){t=document.querySelector("#app header"),t&&(n=t.style.display,t.style.display="none")}function o(){t&&(t.style.display=n??"",t=null,n=null)}}function br(){let a=String(O.contact?.whatsapp??"").replace(/\D/g,"").replace(/^0/,"62");return a?`https://wa.me/${a}`:""}var hr="carlynk.id";function kr(){return`${hr}/s/showroom-anda`}var dt="public.carlynk-landing",Oa=Object.freeze([{name:"public.carlynk-landing",label:"Landing Carlynk",path:"/carlynk-landing",description:"Halaman marketing Carlynk mengikuti berkas desain, menyasar pemilik showroom.",page:at},{name:"public.saas-landing",label:"Landing SaaS Jual Beli Mobil",path:"/saas-landing",description:"Halaman marketing SaaS untuk showroom, marketing, dan buyer.",page:ct},{name:"public.catalog-alias",label:"Katalog Mobil Publik",path:"/public",description:"Halaman katalog mobil lama yang tetap tersedia di #/public.",page:Ne},{name:"public.auth-landing",label:"Login / Register",path:"/auth",description:"Halaman autentikasi publik untuk buyer, showroom, admin, dan marketing.",page:He}]);function No(){return Oa.map(({name:a,label:e,path:t,description:n})=>({name:a,label:e,path:t,description:n}))}function xr(a=""){return Oa.find(e=>e.name===a)??Oa.find(e=>e.name===dt)}function jo(){let a=u.get("working.adminWebConfig.config.data",null),e=u.get("snapshot.admin.webConfig.data",null),t=Ie(),n=a?.landing_page_route_name??e?.landing_page_route_name??t?.landingPage?.routeName??t?.landing_page_route_name??"";return xr(n)?.name??dt}export{Nr as a,na as b,oe as c,Rr as d,Pr as e,ra as f,ce as g,me as h,fe as i,h as j,he as k,Ne as l,He as m,at as n,ct as o,No as p,xr as q,jo as r};
