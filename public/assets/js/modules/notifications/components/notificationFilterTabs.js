...
export function NotificationFilterTabs({ active = "all", onChange = null } = {}) {
  ...
  FILTERS.forEach((filter) => {
    ...
    if (isActive) {
      ...
      button.className =
        "inline-flex min-h-10 min-w-[3.5rem] items-center justify-center rounded-full border border-[var(--pb-border)] bg-[var(--pb-surface-muted)] px-3 py-2 text-xs text-[var(--pb-text-strong)] shadow-[var(--pb-shadow-soft)] transition hover:bg-[color-mix(in_srgb,var(--pb-surface-muted)_90%,var(--pb-text))] focus:outline-none focus:ring-2 focus:ring-[var(--pb-form-focus)]";
    }
