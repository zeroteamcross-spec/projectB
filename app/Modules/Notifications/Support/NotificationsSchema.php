...
class NotificationsSchema
{
    ...
    {
        try {
            $this->bootstrapper->ensureTable('notifications', $this->createNotificationsTableSql());
        } catch (Throwable $exception) {
            error_log('Notifications schema bootstrap failed: ' . $exception->getMessage());

            throw new HttpException(
                'Schema notifikasi belum siap. Periksa privilege CREATE TABLE dan konfigurasi database.',
                500
            );
        }
    }

    private function createNotificationsTableSql(): string
    {
        return <<<'SQL'
