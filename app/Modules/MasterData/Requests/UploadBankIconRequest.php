...
class UploadBankIconRequest
{
    ...
    public function validate(): array
    {
        ...
        if (! is_array($file)) {
            $errors['icon'] = 'Icon bank wajib diupload.';
        } elseif (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            $errors['icon'] = 'Upload icon bank gagal.';
        } else {
            $size = (int) ($file['size'] ?? 0);
            if ($size <= 0) {
                $errors['icon'] = 'File icon bank kosong.';
            } elseif ($size > 2 * 1024 * 1024) {
                $errors['icon'] = 'File icon bank maksimal 2 MB.';
            }
            ...
            if ($isSvg) {
                ...
                if (! $this->isSafeSvg($svg)) {
                    $errors['icon'] = 'SVG icon bank tidak valid atau mengandung konten tidak aman.';
                }
                $mimeType = 'image/svg+xml';
            } elseif (! in_array($mimeType, self::ALLOWED_MIME_TYPES, true)) {
                $errors['mime_type'] = 'Icon bank harus berupa jpeg, png, webp, atau svg.';
            }

            if (! $isSvg) {
                $sizeInfo = @getimagesize((string) ($file['tmp_name'] ?? ''));
                if (! is_array($sizeInfo)) {
                    $errors['icon'] = 'File icon bank bukan gambar valid.';
                } elseif ((int) $sizeInfo[0] < 64 || (int) $sizeInfo[1] < 64) {
                    $errors['dimensions'] = 'Dimensi icon bank minimal 64x64 px.';
                }
            }
        }
